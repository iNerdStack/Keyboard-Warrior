  ...
  <div class="bg-background h-full min-h-screen flex md:justify-center">
    <div class="flex flex-col w-full md:max-w-[1000px]" v-if="words">
      <div class="p-10 flex flex-col h-full">
        ...
        <div
          class="bg-white md:px-8 px-4 flex flex-col block rounded-lg border border-gray-200 shadow-md"
        >
          <div
            :class="`flex flex-row text-black text-center md:text-left h-[30px] justify-center mt-5 scale-100 ${
              isGameOn ? 'scale-75 py-2' : 'scale-100 py-8'
          ...
          >
            <div class="flex flex-row items-center">
              <div>
                <Icon
                  name="mdi:keyboard"
                  class="md:text-[45px] text-[30px] text-secondary mt-1"
                />
              </div>

              <span class="text-[15px] md:text-lg pl-1 md:pl-2">
                {{ CPM }} CPM
              </span>
            </div>
            ...
            <div class="flex flex-row items-center ml-1 md:pl-3">
              ...
              <span class="text-[15px] md:text-lg pl-1 md:pl-2 font-bold">
                {{ time }}
              </span>
            </div>
          ...
          <div>
            <div
              class="mt-5 flex text-center py-5 px-3 mb-3 bg-black rounded-md"
            >
              <span class="mx-1 text-white text-sm md:text-lg">
                <div v-html="computedWordResult" />
              </span>
            </div>
          </div>
        </div>
...
const wordAccuracy = computed<number>(() => {
  ...
  return inputRecordArray.value.length == 0
    ? 100
    : Math.floor((numberOfCorrectWords / inputRecordArray.value.length) * 100);
});
